...
class ReportController extends Controller
{
    ...
    public function patients(Request $request)
    {
        ...
        // Enhanced patient statistics (visible patients)
        $patientStats = [
            'total_patients' => $clinic->patients()->visibleInClinic()->count(),
            'new_patients' => $clinic->patients()->visibleInClinic()->new()->count(),
            'active_patients' => $clinic->patients()->visibleInClinic()->active()->count(),
            'inactive_patients' => $clinic->patients()->visibleInClinic()->inactive()->count(),
            'new_this_month' => $clinic->patients()->visibleInClinic()->whereMonth('created_at', now()->month)->count(),
            'total_revenue' => $clinic->patients()->visibleInClinic()->withSum('payments', 'amount')->get()->sum('payments_sum_amount'),
            'avg_age' => $clinic->patients()->visibleInClinic()->whereNotNull('date_of_birth')->get()->avg(function($patient) {
                return $patient->date_of_birth ? $patient->date_of_birth->age : 0;
            }),
            'verified_emails' => $clinic->patients()->visibleInClinic()->where('email_verified', true)->count(),
            'with_insurance' => $clinic->patients()->visibleInClinic()->whereNotNull('insurance_provider')->count(),
        ];

        // Demographics data (visible patients)
        $demographics = [
            'age_groups' => $clinic->patients()->visibleInClinic()->whereNotNull('date_of_birth')->get()->groupBy(function($patient) {
            ...
            'gender_distribution' => $clinic->patients()->visibleInClinic()->whereNotNull('gender')->get()->groupBy('gender')->map(function($group) {
                return $group->count();
            }),
            'categories' => $clinic->patients()->visibleInClinic()->whereNotNull('category')->get()->groupBy('category')->map(function($group) {
                return $group->count();
            }),
        ];
