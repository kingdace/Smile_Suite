...
class SmtpSslFixServiceProvider extends ServiceProvider
{
    ...
    {
        // Only apply SSL fix in production environment
        if (app()->environment('production')) {
            // Override the SMTP mailer to fix SSL certificate issues for production
            $this->app->afterResolving(MailManager::class, function (MailManager $mailManager) {
                $mailManager->extend('smtp', function (array $config) {
                    // Create a custom stream with SSL verification disabled for production
