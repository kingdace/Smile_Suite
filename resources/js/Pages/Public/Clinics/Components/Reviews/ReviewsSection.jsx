...
export default function ReviewsSection({
    ...
    // Check if current user has already reviewed this clinic
    const hasUserReviewedClinic = () => {
        if (!auth?.user) return false;
        return localReviews.some(
            (review) =>
                !review.staff_id && review.patient?.user?.id === auth.user.id
        );
    };

    // Check if current user has already reviewed a specific doctor
    const hasUserReviewedDoctor = (doctorId) => {
        if (!auth?.user) return false;
        return localReviews.some(
            (review) =>
                review.staff_id === doctorId &&
                review.patient?.user?.id === auth.user.id
        );
    };
    ...
    const handleDoctorReviewClick = (doctor) => {
        ...
        if (auth.user.role !== "patient") {
            showToast("Only patients can write reviews.", "error");
            return;
        }

        // Check if user has already reviewed this doctor
        if (hasUserReviewedDoctor(doctor.id)) {
            showToast("You have already reviewed this doctor.", "error");
            return;
        }
    ...
    const averageRating =
        clinicReviews.reduce((sum, review) => sum + review.rating, 0) /
                                        ...
                                        <div className="text-center py-8">
                                            <div className="mb-6">
                                                ...
                                                <h4 className="text-lg font-semibold text-gray-900 mb-2">
                                                    Sign in to Review Doctors
                                                </h4>
                                                <p className="text-gray-600 text-sm max-w-md mx-auto">
                                                    Help other patients by
                            ...
                            hasUserReviewedClinic() ? (
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-gray-600 text-sm">
                                        ...
                                        Your feedback helps other patients make
                                        informed decisions.
                                    </p>
                                </div>
                            ) : (
